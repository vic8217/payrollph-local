...
export default async function handler(req, res) {
  ...
  const receiptMethod = 'qr_passkey_photo';
  if (!qrCode || !passkey) {
    return res.status(400).json({ error: 'QR code and passkey are required for payslip acknowledgement.' });
  }
  ...
  const updated = await updateRecord('PayrollRecord', payrollRecord.id, {
    ...
    payslip_acknowledgement_face_verified: false,
    payslip_acknowledgement_face_verification_log_id: null,
    payslip_acknowledgement_face_confidence: null,
  ...
  await createRecord('PasscodeAuditLog', {
    ...
    reason: 'Employee QR code, passkey, and live identity photo verified',

...
export default async function handler(req, res) {
  ...
  const ledger = await createRecord("CashAdvanceLedger", {
    ...
    deduction_total: Number(advance.deduction_payroll_periods) || Number(advance.deduction_periods_remaining) || null,
  });
  ...
  await createRecord("PasscodeAuditLog", {
    ...
    reason,
    summary: `Cash advance balance ${adjustmentType}d by ₱${amount.toLocaleString("en-PH", { minimumFractionDigits: 2 })}. Balance: ₱${balanceBefore.toLocaleString("en-PH", { minimumFractionDigits: 2 })} to ₱${balanceAfter.toLocaleString("en-PH", { minimumFractionDigits: 2 })}`,
    employee_id: advance.employee_id,
    employee_name: advance.employee_name,
    amount,

...
const AuthenticatedApp = () => {
  ...
  if (isLoadingPublicSettings || isLoadingAuth) return <div className="fixed inset-0 flex items-center justify-center"><div className="w-8 h-8 border-4 border-slate-200 border-t-primary rounded-full animate-spin"></div></div>;
  if (authError?.type === 'user_not_registered') return <UserNotRegisteredError />;
  if (!user) return <Routes><Route path="/" element={<Landing />} /><Route path="/landing" element={<Landing />} /><Route path="/reset-password" element={<Landing />} /><Route path="/scan/confirm" element={<ScanConfirm />} /><Route path="/employee-portal-qr" element={<EmployeePortalQR />} /><Route path="/employee-portal" element={<EmployeePortal />} /><Route path="*" element={<Navigate to="/" replace />} /></Routes>;
...
function App() { return <AuthProvider><CompanyProvider><QueryClientProvider client={queryClientInstance}><Router future={{ v7_startTransition: true, v7_relativeSplatPath: true }}><AuthenticatedApp /></Router><Toaster /></QueryClientProvider></CompanyProvider></AuthProvider> }
export default App

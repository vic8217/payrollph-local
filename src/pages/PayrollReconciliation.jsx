...
export default function PayrollReconciliation() {
  ...
  useEffect(() => { if (records.length && !records.some(row => String(row.employee_id) === employeeId)) setEmployeeId(String(records[0].employee_id)); }, [records, employeeId]);
  const record = records.find(row => String(row.employee_id) === employeeId);
  const logsQuery = useQuery({ queryKey: ['recon-logs', activeCompanyId, employeeId, period?.start_date, period?.end_date], queryFn: () => appApi.entities.AttendanceLog.filter({ company_profile_id: activeCompanyId, employee_id: employeeId, date: { $gte: period.start_date, $lte: period.end_date } }, 'date', 100), enabled: Boolean(activeCompanyId && employeeId && period?.start_date) });
  const overtimeRequestsQuery = useQuery({ queryKey: ['recon-overtime-requests', activeCompanyId, employeeId, period?.start_date, period?.end_date], queryFn: () => appApi.entities.OvertimeRequest.filter({ company_profile_id: activeCompanyId, employee_id: employeeId, date: { $gte: period.start_date, $lte: period.end_date }, status: 'approved' }, '-reviewed_at', 100), enabled: Boolean(activeCompanyId && employeeId && period?.start_date) });
  const reconQuery = useQuery({ queryKey: ['recon-saved', activeCompanyId, periodId, employeeId], queryFn: () => appApi.entities.PayrollReconciliation.filter({ company_profile_id: activeCompanyId, payroll_period_id: periodId, employee_id: employeeId }, '-updated_date', 1), enabled: Boolean(activeCompanyId && periodId && employeeId) });
  ...
  useEffect(() => {
    ...
    setManual(existing?.manual_values || Object.fromEntries(SUMMARY_FIELDS.map(([key]) => [key, num(record[key])])));
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      const payload = { company_profile_id: activeCompanyId, payroll_period_id: periodId, period_name: period.period_name || `${period.start_date} to ${period.end_date}`, employee_id: record.employee_id, employee_name: record.employee_name, payroll_record_id: record.id, system_values: Object.fromEntries(SUMMARY_FIELDS.map(([key]) => [key, num(record[key])])), manual_values: manual, system_daily_values: Object.fromEntries(days.map(date => { const log = logByDate.get(date) || {}; return [date, { ...log, overtime_hours: reconciledOvertimeHours(log, overtimeRequests) }]; })), manual_daily_values: manualDays, variance_note: note, reviewed_by: user?.full_name || user?.name || user?.email || 'Unknown officer', reviewed_at: new Date().toISOString() };
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['recon-saved', activeCompanyId, periodId, employeeId] }); setSaved(true); },
  });

  const loading = periodsQuery.isLoading || recordsQuery.isLoading || logsQuery.isLoading || overtimeRequestsQuery.isLoading || reconQuery.isLoading;
  return <div className="w-full space-y-4 p-4 md:p-6">
    ...
    {payrollOvertimeIsStale && <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">Approved attendance contains {currentOvertimeHours.toFixed(2)} OT hours, while this generated payroll contains {num(record.overtime_hours).toFixed(2)}. Regenerate payroll to update the payroll summary and OT pay.</div>}
    {!loading && !record && <Card className="p-10 text-center text-sm text-muted-foreground">Generate payroll for the selected period before starting reconciliation.</Card>}
    {record && <>
      <Card className="overflow-hidden"><div className="border-b px-4 py-3"><h2 className="font-semibold">{record.employee_name}</h2><p className="text-xs text-muted-foreground">{record.department || 'Unassigned'} · System / Manual / Difference (System − Manual)</p></div><div className="overflow-x-auto"><table className="w-full min-w-[1800px] text-xs"><thead className="bg-muted/70"><tr><th className="sticky left-0 z-10 bg-muted px-3 py-2 text-left">Source</th>{SUMMARY_FIELDS.map(([key,label]) => <th key={key} className="whitespace-nowrap px-3 py-2 text-right">{label}</th>)}</tr></thead><tbody><tr className="border-t"><td className="sticky left-0 bg-background px-3 py-2 font-bold text-red-600">System</td>{SUMMARY_FIELDS.map(([key,,type]) => <td key={key} className="px-3 py-2 text-right font-mono">{display(record[key],type)}</td>)}</tr><tr className="border-t"><td className="sticky left-0 bg-background px-3 py-2 font-bold text-blue-600">Manual</td>{SUMMARY_FIELDS.map(([key]) => <td key={key} className="p-1"><Input type="number" step="0.01" className="h-8 min-w-24 text-right font-mono" value={manual[key] ?? ''} onChange={event => setManual(values => ({ ...values, [key]: event.target.value }))}/></td>)}</tr><tr className="border-t bg-yellow-50"><td className="sticky left-0 bg-yellow-50 px-3 py-2 font-bold text-amber-700">Difference</td>{SUMMARY_FIELDS.map(([key,,type]) => { const difference = num(record[key])-num(manual[key]); return <td key={key} className={`px-3 py-2 text-right font-mono font-semibold ${Math.abs(difference) > .005 ? 'bg-red-100 text-red-700' : 'text-emerald-700'}`}>{display(difference,type)}</td>; })}</tr></tbody></table></div></Card>

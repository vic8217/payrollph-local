...
export default function PayrollReconciliation() {
  ...
  useEffect(() => {
    ...
    setManualDays(existing?.manual_daily_values || Object.fromEntries(days.map(date => {
      ...
      return [date, { day_type: log.day_type || '', time_in: timeValue(log.time_in), break_time_out: timeValue(log.break_time_out), break_time_in: timeValue(log.break_time_in), time_out: timeValue(log.time_out), ...Object.fromEntries(DAILY_FIELDS.map(([key]) => [key, num(key === 'regular_hours' ? log.hours_worked : log[key])])) }];
    })));
    setNote(existing?.variance_note || '');
    setSaved(false);
  }, [record?.id, existing?.id, reconQuery.isLoading, days.join('|'), logsQuery.dataUpdatedAt]);

  const saveMutation = useMutation({
    mutationFn: () => {
      const payload = { company_profile_id: activeCompanyId, payroll_period_id: periodId, period_name: period.period_name || `${period.start_date} to ${period.end_date}`, employee_id: record.employee_id, employee_name: record.employee_name, payroll_record_id: record.id, system_values: Object.fromEntries(SUMMARY_FIELDS.map(([key]) => [key, num(record[key])])), manual_values: manual, system_daily_values: Object.fromEntries(days.map(date => [date, logByDate.get(date) || {}])), manual_daily_values: manualDays, variance_note: note, reviewed_by: user?.full_name || user?.name || user?.email || 'Unknown officer', reviewed_at: new Date().toISOString() };
  ...
  const loading = periodsQuery.isLoading || recordsQuery.isLoading || logsQuery.isLoading || reconQuery.isLoading;

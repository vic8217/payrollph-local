...
function ConsolidatedReconciliation({ records, reconciliations, onEmployee, period }) {
  ...
  const reviewed = rows.filter(row => row.reconciliation && (!row.variance || ['accept_system', 'accept_manual'].includes(row.reconciliation?.resolution_status))).length;
  const unresolved = rows.filter(row => row.variance > 0 && !['accept_system', 'accept_manual'].includes(row.reconciliation?.resolution_status)).length;

  ...
  return (
    <div className="w-full p-4 sm:p-6 space-y-4 sm:space-y-6">
      ...
      <div>
        ...
        <Button variant="outline" className="ml-2 gap-2 border-amber-300 text-amber-700 hover:bg-amber-50" onClick={() => setAttendanceDetail('late')}>
      ...
      <Dialog open={Boolean(attendanceDetail)} onOpenChange={open => !open && setAttendanceDetail(null)}><DialogContent className="max-w-4xl"><DialogHeader><DialogTitle>{attendanceDetail === 'absent' ? 'Absent Details' : 'Late Details'}</DialogTitle></DialogHeader>{(() => { const isLateDetail = attendanceDetail === 'late'; const records = isLateDetail ? currentLate : currentAbsent; const previousRecords = isLateDetail ? previousLate : previousAbsent; const title = isLateDetail ? 'Late records' : 'Absent records'; return <div className="space-y-5"><div className="grid gap-3 sm:grid-cols-2"><Card className="p-3"><p className="text-xs text-muted-foreground">Current payroll period</p><p className="font-semibold">{latestPayroll?.period_name || '—'}</p><p className={`text-2xl font-bold ${isLateDetail ? 'text-amber-700' : 'text-red-700'}`}>{records.length}</p><p className="text-xs text-muted-foreground">{title.toLowerCase()}</p></Card><Card className="p-3"><p className="text-xs text-muted-foreground">Previous payroll period</p><p className="font-semibold">{previousPayroll?.period_name || '—'}</p><p className="text-2xl font-bold">{previousRecords.length}</p><p className="text-xs text-muted-foreground">{title.toLowerCase()}</p></Card></div><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-muted"><tr>{['Name','Employee No.','Date','Payroll Period','Status','Late Minutes'].map(label => <th key={label} className="px-3 py-2 text-left">{label}</th>)}</tr></thead><tbody>{records.map(record => <tr key={record.id} className="border-t"><td className="px-3 py-2 font-medium">{record.employee_name || '—'}</td><td className="px-3 py-2">{record.employee_id || '—'}</td><td className="px-3 py-2">{record.date || '—'}</td><td className="px-3 py-2">{latestPayroll?.period_name || '—'}</td><td className={`px-3 py-2 font-semibold ${isLateDetail ? 'text-amber-600' : 'text-red-600'}`}>{isLateDetail ? 'Late' : 'Absent'}</td><td className="px-3 py-2">{isLateDetail ? Number(record.late_minutes || 0) : '—'}</td></tr>)}{!records.length && <tr><td colSpan="6" className="px-3 py-6 text-center text-muted-foreground">No {title.toLowerCase()} in the current payroll period.</td></tr>}</tbody></table></div></div>; })()}</DialogContent></Dialog>

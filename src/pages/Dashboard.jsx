...
export default function Dashboard() {
  ...
  const [lateEmployeeId, setLateEmployeeId] = useState(null);
  ...
  return (
    <div className="w-full p-4 sm:p-6 space-y-4 sm:space-y-6">
      ...
      <div>
        ...
        <Button variant="outline" className="ml-2 gap-2 border-amber-300 text-amber-700 hover:bg-amber-50" onClick={() => { setLateEmployeeId(null); setAttendanceDetail('late'); }}>
          <AlertTriangle className="w-4 h-4" /> Late Details
        </Button>
        <Button variant="outline" className="ml-2 gap-2 border-red-300 text-red-700 hover:bg-red-50" onClick={() => setAttendanceDetail('absent')}>
          <AlertTriangle className="w-4 h-4" /> Absent Details
        </Button>
      </div>
      ...
      <Dialog open={Boolean(attendanceDetail)} onOpenChange={open => { if (!open) { setAttendanceDetail(null); setLateEmployeeId(null); } }}><DialogContent className="max-w-4xl"><DialogHeader><DialogTitle>{attendanceDetail === 'absent' ? 'Absent Details' : lateEmployeeId ? 'Late Days by Employee' : 'Late Details'}</DialogTitle></DialogHeader>{(() => { const isLateDetail = attendanceDetail === 'late'; const records = isLateDetail ? currentLate : currentAbsent; const previousRecords = isLateDetail ? previousLate : previousAbsent; const title = isLateDetail ? 'Late days' : 'Absent days'; const lateEmployees = Object.values(currentLate.reduce((groups, record) => { const key = String(record.employee_id || record.id); if (!groups[key]) groups[key] = { key, name: record.employee_name || '—', employeeNumber: record.employee_id || '—', records: [], totalMinutes: 0 }; groups[key].records.push(record); groups[key].totalMinutes += Number(record.late_minutes || 0); return groups; }, {})).sort((a, b) => b.records.length - a.records.length || a.name.localeCompare(b.name)); const selectedLateEmployee = lateEmployees.find(item => item.key === lateEmployeeId); const detailRecords = selectedLateEmployee?.records || records; return <div className="space-y-5"><div className="grid gap-3 sm:grid-cols-3"><Card className="p-3"><p className="text-xs text-muted-foreground">Current payroll period</p><p className="font-semibold">{latestPayroll?.period_name || '—'}</p><p className={`text-2xl font-bold ${isLateDetail ? 'text-amber-700' : 'text-red-700'}`}>{records.length}</p><p className="text-xs text-muted-foreground">{title}</p></Card>{isLateDetail && <Card className="p-3"><p className="text-xs text-muted-foreground">Employees late</p><p className="mt-5 text-2xl font-bold text-amber-700">{lateEmployees.length}</p><p className="text-xs text-muted-foreground">Distinct employees</p></Card>}<Card className="p-3"><p className="text-xs text-muted-foreground">Previous payroll period</p><p className="font-semibold">{previousPayroll?.period_name || '—'}</p><p className="text-2xl font-bold">{previousRecords.length}</p><p className="text-xs text-muted-foreground">{title}</p></Card></div>{isLateDetail && !lateEmployeeId ? <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-muted"><tr>{['Employee','Employee No.','Late Days','Total Late Minutes','Action'].map(label => <th key={label} className="px-3 py-2 text-left">{label}</th>)}</tr></thead><tbody>{lateEmployees.map(item => <tr key={item.key} className="border-t"><td className="px-3 py-2 font-medium">{item.name}</td><td className="px-3 py-2">{item.employeeNumber}</td><td className="px-3 py-2 font-semibold text-amber-700">{item.records.length}</td><td className="px-3 py-2">{item.totalMinutes}</td><td className="px-3 py-2"><Button size="sm" variant="outline" onClick={() => setLateEmployeeId(item.key)}>View days</Button></td></tr>)}{!lateEmployees.length && <tr><td colSpan="5" className="px-3 py-6 text-center text-muted-foreground">No late days in the current payroll period.</td></tr>}</tbody></table></div> : <><div className="flex items-center justify-between"><div>{selectedLateEmployee && <><p className="font-medium">{selectedLateEmployee.name}</p><p className="text-xs text-muted-foreground">{selectedLateEmployee.employeeNumber} · {selectedLateEmployee.records.length} late day{selectedLateEmployee.records.length === 1 ? '' : 's'}</p></>}</div>{selectedLateEmployee && <Button size="sm" variant="outline" onClick={() => setLateEmployeeId(null)}>Back to summary</Button>}</div><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-muted"><tr>{['Name','Employee No.','Date','Payroll Period','Status','Late Minutes'].map(label => <th key={label} className="px-3 py-2 text-left">{label}</th>)}</tr></thead><tbody>{detailRecords.map(record => <tr key={record.id} className="border-t"><td className="px-3 py-2 font-medium">{record.employee_name || '—'}</td><td className="px-3 py-2">{record.employee_id || '—'}</td><td className="px-3 py-2">{record.date || '—'}</td><td className="px-3 py-2">{latestPayroll?.period_name || '—'}</td><td className={`px-3 py-2 font-semibold ${isLateDetail ? 'text-amber-600' : 'text-red-600'}`}>{isLateDetail ? 'Late' : 'Absent'}</td><td className="px-3 py-2">{isLateDetail ? Number(record.late_minutes || 0) : '—'}</td></tr>)}{!detailRecords.length && <tr><td colSpan="6" className="px-3 py-6 text-center text-muted-foreground">No {title.toLowerCase()} in the current payroll period.</td></tr>}</tbody></table></div></>}</div>; })()}</DialogContent></Dialog>

...
import { Landmark, ShieldCheck, HeartPulse, Home, Calculator } from 'lucide-react';
import { Card } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { computePagIbig, computePhilHealth, computeSSS } from '@/lib/payrollUtils';
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-0.5">Review the SSS, PhilHealth, and Pag-IBIG contribution rules used by payroll.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <RateCard icon={HeartPulse} title="PhilHealth">
          ...
          <Row label="Salary credit range" value="₱10,000 - ₱80,000" />
        </RateCard>

        <RateCard icon={Home} title="Pag-IBIG">
          <Row label="Employee rate" value="1% up to ₱1,500, else 2%" />
          <Row label="Employer rate" value="2%" />
          <Row label="App cap" value="₱100 per month" />
          <Row label="Employee max" value={formatPeso(computePagIbig(5000).employee)} />
        </RateCard>
      </div>

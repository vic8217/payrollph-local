...
function isIncompletePunch(log) {
  ...
  // A half-day may be recorded as one continuous in/out span. Break punches
  // are optional in that case, even when the employee's default shift has a
  // lunch-break schedule. This also supports older logs that were not tagged
  // with `half_day` but have a short continuous worked span.
  const noBreakPunches = !log.break_time_out && !log.break_time_in;
  const elapsedHours = noBreakPunches
    ? (new Date(log.time_out).getTime() - new Date(log.time_in).getTime()) / (60 * 60 * 1000)
    : 0;
  const isHalfDay = String(log.day_type || '').toLowerCase() === 'half_day'
    || (noBreakPunches && Number.isFinite(elapsedHours) && elapsedHours > 0 && elapsedHours <= 6.5);
